...
private const val NEXT_MATCH = "nextMatch"
private const val leagueId: Int = 4332

class NextMatchFragment : Fragment(), MainView {
    private var events: MutableList<Event> = mutableListOf()
    private var listener: interactionListenerNext? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        arguments?.let {
            match = it.getBoolean(NEXT_MATCH)
        }
    ...
    override fun onAttach(context: Context?) {
        ...
        if (context is interactionListenerNext) {
            listener = context
        } else {
    ...
    interface interactionListenerNext {
        fun itemClickNext(match: Event)
    }

    companion object {
        @JvmStatic
        fun newInstanceNext(match: Boolean) =
            NextMatchFragment().apply {
                arguments = Bundle().apply {
                    putBoolean(NEXT_MATCH, match)
                }
            }
    }
